...
Static Function ReportPrint(oReport,oCabec,oPedaFat,oTemp,cAliasSC5,cAliasSC6,cAliasSC9,cAliasSF4)
	...
	oELT := FWTemporaryTable():New("STR")
	oELT:SetFields(aCampos)
	oELT:AddIndex("01",{"TB_LOCAL", "TB_COD"})
	oELT:Create()
				...
				If Empty(_TBcFiltro)
					BeginSql Alias cAliasSC5
						SELECT
							...
							SC5.C5_ACRSFIN
						FROM
							%Table:SC5% SC5,%Table:SC6% SC6,%Table:SC9% SC9,%Table:SF4% SF4
						WHERE
							...
							AND SC6.C6_PRODUTO >= %Exp:mv_par03%
							AND SC6.C6_PRODUTO <= %Exp:mv_par04%
							AND SC6.C6_ENTREG >= %Exp:dtos(mv_par10)%
							AND SC6.C6_ENTREG <= %Exp:dtos(mv_par11)%
						...
						SELECT
							...
							SC5.C5_ACRSFIN
						FROM
							%Table:SC5% SC5,%Table:SC6% SC6,%Table:SF4% SF4
						WHERE
							SC5.C5_FILIAL = %xFilial:SC5%
							AND SC5.C5_NUM >= %Exp:mv_par01%
							AND SC5.C5_NUM <= %Exp:mv_par02%
