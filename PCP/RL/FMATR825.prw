...
Static Function ReportDef()
	...
	If ValType(aRetSX5) == "A" .AND. Len(aRetSX5) >= 1
		for nX :=  1 to len(aRetSX5)
			TRCell():New(oApont,Trim(aRetSX5[3]),,Trim(aRetSX5[3]),cPictH6,,,)
		NEXT nX
	ENDIF
...
Static Function ReportPrint(oReport,aOrdem)
	...
	If ValType(aRetSX5) == "A" .AND. Len(aRetSX5) >= 1
		for nX :=  1 to len(aRetSX5)
			oFunction := TRFunction():New(oApont:Cell(Trim(aRetSX5[3])),Trim(aRetSX5[3]),If(cTpHr =="N","TIMESUM","SUM"),,Padr(Trim(Upper(aRetSX5[4])),35,"-"),,,.F.,.T.)
		NEXT nX
	ENDIF
	...
	#IFDEF TOP
		...
		BeginSql Alias cAliasSH6
			...
			FROM
				%table:SH6% SH6
			WHERE
				H6_FILIAL = %xFilial:SH6%
				AND //H6_OP       Between %Exp:mv_par02% AND %Exp:mv_par03% AND
				H6_RECURSO Between %Exp:mv_par04% AND %Exp:mv_par05%
				AND H6_MOTIVO Between %Exp:mv_par06% AND %Exp:mv_par07%
				AND H6_DTAPONT Between %Exp:mv_par08% AND %Exp:mv_par09%
				AND H6_OPERADO Between %Exp:mv_par10% AND %Exp:mv_par11%
				AND //%Exp:cWhere cWhere1%
				%Exp:cWhere2% SH6.%NotDel%
			ORDER BY
				%Exp:cOrderBy%
		EndSql
	...
	If ValType(aRetSX5) == "A" .AND. Len(aRetSX5) >= 1
		for nX :=  1 to len(aRetSX5)
			oApont:Cell(Trim(aRetSX5[3])):Hide()
			oApont:Cell(Trim(aRetSX5[3])):HideHeader()
		NEXT nX
	ENDIF
	...
	While !oReport:Cancel() .And. !(cAliasSH6)->(Eof())
		...
		If ValType(aRetSX5) == "A" .AND. Len(aRetSX5) >= 1
			for nX :=  1 to len(aRetSX5)
				dbSelectArea(cAliasSH6)
				If (cAliasSH6)->H6_MOTIVO == Trim(aRetSX5[3])
					oApont:Cell(Trim(aRetSX5[3])):SetValue(Val(StrTran(TimeH6(Nil,Nil,cAliasSH6),':','.')))
				ELSE
					oApont:Cell(Trim(aRetSX5[3])):SetValue(0)
				ENDIF
			NEXT nX
		ENDIF
	...
	If ValType(aRetSX5) == "A" .AND. Len(aRetSX5) >= 1
		for nX :=  1 to len(aRetSX5)
			xValMot := oApont:GetFunction(Trim(aRetSX5[3])):ReportValue()
