		...
		If SH6->H6_QTDPERD > 0
			...
			Begin Transaction
				...
				While SG1->(!EoF()) .And. SG1->G1_COD == SC2->C2_PRODUTO
					If (Select("Z05ALIAS") > 0)
						DBSelectArea("Z05ALIAS")
						Z05ALIAS->(DBCloseArea())
					EndIf
					BEGINSQL ALIAS "Z05ALIAS"
						SELECT Z05.Z05_PERDA PERDA FROM %TABLE:SB1% B1
						INNER JOIN %TABLE:Z05% Z05 ON Z05.Z05_FILIAL=B1.B1_FILIAL
						AND Z05.%NOTDEL% AND B1.B1_BRTPPR=Z05.Z05_NOME
						WHERE B1.B1_FILIAL=%XFILIAL:SB1%
						AND B1.%NOTDEL%
						AND B1.B1_BRTPPR <> ''
						AND B1.B1_COD = %EXP:SG1->G1_COMP%
					ENDSQL
					a_Vetor := {}
					dbSelectArea("SD4")
					SD4->(dbSetOrder(2))
					SD4->(dbSeek(xFilial("SD4") + SH6->H6_OP + SG1->G1_COMP))
					If (Found() .AND. Z05ALIAS->PERDA == 'S')
